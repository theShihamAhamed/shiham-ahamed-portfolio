...
const getFeaturedProjects = async (): Promise<FeaturedProjectsState> => {
  try {
    const projects = await getFeaturedProjectsFromMongo();

    return {
      projects: projects.slice(0, 6).map(mapPublicProjectToViewerProject),
      error: null,
    };
  } catch (error) {
    ...
    return {
      projects: [],
      error: "Featured projects are temporarily unavailable.",
    };
  }
};

const FeaturedProjectsSection = async () => {
  const { projects: featuredProjects, error } = await getFeaturedProjects();
