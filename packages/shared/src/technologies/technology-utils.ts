...
export const searchTechnologies = (query: string, category?: TechTagCategory) => { const q = normalizeTechnologyTag(query); return TECHNOLOGY_REGISTRY.filter((entry) => !category || entry.category === category).map((entry) => ({ entry, rank: !q ? 4 : normalizeTechnologyTag(entry.label) === q ? 0 : entry.aliases.some((a) => normalizeTechnologyTag(a) === q) ? 1 : normalizeTechnologyTag(entry.label).startsWith(q) ? 2 : [entry.label, entry.slug, ...entry.aliases].some((v) => normalizeTechnologyTag(v).includes(q)) ? 3 : 99 })).filter((item) => item.rank < 99).sort((a, b) => a.rank - b.rank || a.entry.label.localeCompare(b.entry.label)).map(({ entry }) => entry); };
